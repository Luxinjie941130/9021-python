...
class BinaryTree:
    ...
    def occurs_in_tree(self, value): ##（）里的值是否在树中存在
        '''
        >>> t = BinaryTree()
        >>> t.occurs_in_tree(0)
        False
    ...
    def occurs_in_bst(self, value):  ###（）不存在为false，存在为true，（）值小于root值，返回左，大于，返回右
        '''
        >>> t = BinaryTree()
        >>> t.occurs_in_bst(0)
        False
    ...
    def in_order_traversal(self): ###节点值按从小到大排序
        '''
        >>> t = BinaryTree()
